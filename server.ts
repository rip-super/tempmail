...
app.post("/inbox/:address/add", async c => {
    ...
    const { senderName, senderEmail, subject, textBody, htmlBody } = await c.req.json();
    ...
    const email: Email = {
        ...
        textBody: await encryptField(textBody ?? "", pubKey),
        htmlBody: await encryptField(htmlBody ?? "", pubKey),
        receivedAt: Date.now(),
    };
